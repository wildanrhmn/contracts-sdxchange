...
contract P2PDataTransfer is IP2PDataTransfer, ReentrancyGuard, Ownable, Pausable {
    ...
    event TransferInitiated(bytes32 indexed transferId, address indexed sender, address indexed receiver, uint256 totalChunks);
    ...
    function initiateTransfer(
        ...
        if (_encryptedKey.length == 0) revert DataMarketErrors.InvalidTransfer();

        bytes32 transferId = keccak256(
            abi.encodePacked(msg.sender, _receiver, block.timestamp, _dataHash)
        );

        uint256 totalChunks = (_totalSize + MAX_CHUNK_SIZE - 1) / MAX_CHUNK_SIZE;
        ...
        transfer.confirmedChunkCount = 0;
    ...
    function getTransferProgress(
        ...
        uint256 timeLeft = 0;
        if (!transfer.isCompleted && block.timestamp < transfer.startTime + TRANSFER_TIMEOUT) {
            timeLeft = (transfer.startTime + TRANSFER_TIMEOUT) - block.timestamp;
        }
